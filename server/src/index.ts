...
const deckSchema = z.object({
	...
	cards: z.array(cardScemea).min(10).max(20).describe('Array of flashcards (aim for 20)'),
});

type Deck = z.infer<typeof deckSchema>;
type Card = z.infer<typeof cardScemea>;
...
export default {
	async fetch(request, env, ctx): Promise<Response> {
		...
		registerAppResource(server, 'Flashcard Widget', WIDGET_URI, { description: 'Flashcard Widget' }, async () => {
			const html = await env.ASSETS.fetch(new URL('http://your-adb.com/index.html'));
			return {
				contents: [
					{
						uri: WIDGET_URI,
		...
		registerAppTool(
			...
			async ({ username }) => {
				...
				for (const deckId of deckIds) {
					const deck = await env.FLASHCARDS_KV.get<Deck>(`user:${username}:deck:${deckIds}`, 'json');
					if (deck) {
